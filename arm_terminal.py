...
#region Hand Control
    ...
    def hands_halfway(self):
        # TODO: something wrong with this in mujoco
    ...
    def hands_manual(self):
        ...
        # TODO: fix redundant code (self.hand_control())
        print(f"{self.hand_r.low_state.angle_act=}")
        l_r = input("(L)eft / (R)ight / (B)oth: ")
...
#region Pose Set
    ...
    def pose_push_button(self):
        ...
        # 0 makes self.arms point forwards, as opposed to down with BodyController
        angles_arm = [0] * 29
        if l_r.lower() == "r":
...
#region Misc
    def test(self):
        ...
        # if this doesnt work, try increasing kp or using motor mode instead
        current_angles = [motor.q for motor in body_controller.low_state.motor_state]
        body_controller.low_cmd_control(
            target_angles=current_angles,
            kp=body_controller.kp,
            kd=body_controller.kd,
            duration=0.5
        )
...
if __name__ == "__main__":
    ...
    option_menu = [
        ...
        ("Arm: manual", suite.arm_manual),
        ("Pose set: zero", suite.pose_zero),
        ("Pose set: default", suite.pose_default),
